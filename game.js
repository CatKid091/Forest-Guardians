window.addEventListener("error", function(e){
  ...
  if(box) box.textContent="Game error: "+(e.message||"Unknown error")+" — check that all 4 files are from the same ZIP.";
});

document.addEventListener("DOMContentLoaded", function(){
const canvas=document.getElementById("game"),ctx=canvas.getContext("2d");ctx.imageSmoothingEnabled=false;const W=960,H=600;
const MAPS={
 forest:{title:"🌲 Verdant Forest",sub:"Dense woodland · Moderate challenge",bg:"forest",path:[{x:-20,y:110},{x:170,y:110},{x:170,y:245},{x:390,y:245},{x:390,y:105},{x:650,y:105},{x:650,y:400},{x:880,y:400},{x:980,y:400}],rocks:[{x:95,y:355,r:27},{x:285,y:515,r:30},{x:535,y:500,r:26},{x:790,y:180,r:29}],towers:{oak:{name:"🌳 Oak Guardian",cost:100,range:145,damage:28,fireRate:78,color:"#79552f",bullet:"#b5e36a"},sunflower:{name:"🌻 Sunflower",cost:40,range:115,damage:8,fireRate:34,color:"#f5cf49",bullet:"#fff2a0"},mushroom:{name:"🍄 Mushroom",cost:65,range:105,damage:14,fireRate:58,splash:45,color:"#c873cf",bullet:"#e9a7ed"}}},
 desert:{title:"🏜️ Sunbaked Desert",sub:"Hot sands · Faster pests",bg:"desert",path:[{x:-20,y:430},{x:150,y:430},{x:150,y:180},{x:350,y:180},{x:350,y:400},{x:600,y:400},{x:600,y:150},{x:820,y:150},{x:980,y:300}],rocks:[{x:70,y:150,r:28},{x:270,y:500,r:31},{x:510,y:260,r:27},{x:770,y:470,r:30}],towers:{saguaro:{name:"🌵 Saguaro",cost:45,range:125,damage:9,fireRate:32,color:"#4f8b4a",bullet:"#b7dc7b"},prickly:{name:"🌵 Prickly Pear",cost:70,range:110,damage:15,fireRate:58,splash:42,color:"#d87575",bullet:"#ffd0a8"},creosote:{name:"🌿 Creosote Bush",cost:105,range:150,damage:31,fireRate:78,color:"#a58b46",bullet:"#eadb8a"}}},
 snow:{title:"❄️ Frozen Tundra",sub:"Icy terrain · Tough challenge",bg:"snow",path:[{x:-20,y:160},{x:190,y:160},{x:190,y:350},{x:410,y:350},{x:410,y:130},{x:690,y:130},{x:690,y:450},{x:980,y:450}],rocks:[{x:85,y:400,r:29},{x:300,y:100,r:27},{x:555,y:500,r:32},{x:825,y:250,r:28}],towers:{spruce:{name:"🌲 Spruce",cost:50,range:125,damage:9,fireRate:30,color:"#56866a",bullet:"#d9f4d2"},willow:{name:"🌿 Arctic Willow",cost:75,range:115,damage:16,fireRate:54,splash:38,color:"#9fb77d",bullet:"#eef6d2"},bearberry:{name:"🫐 Bearberry",cost:110,range:155,damage:34,fireRate:82,color:"#7f526b",bullet:"#f2b2d0"}}}
};
let mapKey="forest",map, path,rocks,towerDefs;let mana,lives,wave,score,towers,enemies,bullets,particles,waveActive,spawnLeft,spawnTimer,selectedType,selectedTower,gameOver,gameSpeed;
const $=id=>document.getElementById(id);
function chooseMap(k){mapKey=k;map=MAPS[k];path=map.path;rocks=map.rocks;towerDefs=map.towers;$('mainMenu').classList.add('hidden');$('gameScreen').classList.remove('hidden');$('gameTitle').textContent=map.title;$('gameSubtitle').textContent=map.sub;buildTowerButtons();reset();}
function buildTowerButtons(){const box=$('towerButtons');box.innerHTML="";Object.entries(towerDefs).forEach(([key,d],i)=>{const b=document.createElement('button');b.className='tower'+(i===0?' selected':'');b.dataset.type=key;b.innerHTML=`${d.name}<br><small>Cost ${d.cost} · ${d.splash?'Splash':'Single target'}</small>`;b.onclick=()=>{selectedType=key;selectedTower=null;selectButtons();showBuildMenu()};box.appendChild(b);});}
...
canvas.onclick=e=>{if(gameOver)return;const r=canvas.getBoundingClientRect(),x=(e.clientX-r.left)*W/r.width,y=(e.clientY-r.top)*H/r.height,clicked=towerAt(x,y);if(clicked){selectedTower=clicked;showTowerMenu();msg(`${clicked.def.name} selected.`);return}if(selectedTower){selectedTower=null;showBuildMenu()}const d=towerDefs[selectedType];if(mana<d.cost){msg('Not enough mana!');return}if(pointOnPath(x,y)||rockAt(x,y)||towers.some(t=>dist(t,{x,y})<34)){msg(rockAt(x,y)?'You cannot build on a rock!':'That spot is occupied or on the path!');return}towers.push({x,y,type:selectedType,def:{...d},cool:0,level:1,invested:d.cost});mana-=d.cost;updateHud()};
function showBuildMenu(){$('buildMenu').classList.remove('hidden');$('towerMenu').classList.add('hidden')}function showTowerMenu(){$('buildMenu').classList.add('hidden');$('towerMenu').classList.remove('hidden');updateTowerMenu()}function upgradeCost(t){return Math.round(t.def.cost*(.8+t.level*.65))}function upgradeSelected(){if(!selectedTower||selectedTower.level>=3)return;const c=upgradeCost(selectedTower);if(mana<c){msg('Not enough mana for that upgrade!');return}mana-=c;selectedTower.invested+=c;selectedTower.level++;selectedTower.def.damage=Math.round(selectedTower.def.damage*1.35);selectedTower.def.range=Math.round(selectedTower.def.range*1.08);selectedTower.def.fireRate=Math.max(10,Math.round(selectedTower.def.fireRate*.86));if(selectedTower.def.splash)selectedTower.def.splash+=10;msg(`${selectedTower.def.name} upgraded to Level ${selectedTower.level}!`);updateHud()}function sellSelected(){if(!selectedTower)return;const refund=Math.floor(selectedTower.invested*.7),name=selectedTower.def.name;towers=towers.filter(t=>t!==selectedTower);selectedTower=null;mana+=refund;showBuildMenu();msg(`${name} removed. +${refund} mana refunded.`);updateHud()}function updateTowerMenu(){if(!selectedTower)return;const t=selectedTower;$('towerName').textContent=t.def.name;$('towerLevel').textContent=`LEVEL ${t.level} / 3`;$('towerStats').innerHTML=`⚔ Damage: <b>${t.def.damage}</b><br>🎯 Range: <b>${t.def.range}</b><br>⚡ Attack speed: <b>${Math.max(1,Math.round(60/t.def.fireRate*10)/10)}/sec</b>${t.def.splash?`<br>💥 Splash: <b>${t.def.splash}</b>`:''}`;const u=$('upgradeTower');if(t.level>=3){u.textContent='MAX LEVEL';u.disabled=true}else{const c=upgradeCost(t);u.textContent=`Upgrade — ${c} 💧`;u.disabled=mana<c}}
...
map=MAPS.forest;path=map.path;rocks=map.rocks;towerDefs=map.towers;reset();draw();loop();
